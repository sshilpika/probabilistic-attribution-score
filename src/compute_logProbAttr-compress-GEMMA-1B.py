from log_attr_scores_gemma_1B import ProbabilisticAttributionScores
import torch
...
dynamo.config.fail_on_recompile_limit_hit = False  # avoid hard failure
dynamo.config.recompile_limit = 256                 # default is small (often 8)
    ...
    try:
        for s_id, prompt in enumerate(prompts):
            ...
            if "gpt-neo" in model_name:
                ...
                model = AutoModelForCausalLM.from_pretrained(model_name, token=os.environ['HUGGINGFACEHUB_API_TOKEN'], attn_implementation="eager").eval()
                model.to(device)
            else:
                tokenizer = AutoTokenizer.from_pretrained(model_name, token=os.environ['HUGGINGFACEHUB_API_TOKEN'])
                model = AutoModelForCausalLM.from_pretrained(model_name, token=os.environ['HUGGINGFACEHUB_API_TOKEN']).eval()
                model.to(device)
            ...
            pa = ProbabilisticAttributionScores(model, tokenizer, prompt, device=device)
            
            il, rl, respSQ = pa.get_responses(generate_response=True, get_response_sequences=True)
            response_logits_max = torch.max(rl, dim=-1) #getting max logits for responses
            response = tokenizer.decode(response_logits_max.indices[0])
            
            A_num = torch.log( torch.exp( torch.sum( torch.log( response_logits_max.values ) ) ) )
            ...
            if "allenai" in model_name:
                ...
                step=100
            elif "google/gemma-3-1b-it" in model_name:
                step=50
            elif "meta-llama/Llama-3.2-1B-Instruct" in model_name:
                step = 200

            assert(len(replacement_token_ids)%step == 0)
                
            tensor_path = "../results/one_shot/torch_tensors/"
            ...
            if distributed_state.is_main_process:
                ...
                st = time.time()
                with open(f"../results/one_shot/ALLENAI_{model_save}_{s_id}.csv", 'a') as f:
            ...
            for mu in range(first_mu, input_token_len):
                ...
                # replace the token at location mu which corresponds to mu_val
                ...
                with distributed_state.split_between_processes(inputs_batch, apply_padding=False) as batched_prompts:
                    ...
                    for batch in batched_prompts:
                        ...
                        try:
                            ...
                            print(f"MY: Caught an exception in possibly corrupt batch, {e}")
